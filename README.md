...
<style>
  :root{
    --bg0:#03040c;--bg1:#070b1c;--ink:#f4f7ff;--blue:#6fc2ff;--teal:#2dd4bf;--violet:#8b5cf6;--sky:#38bdf8;
  ...
  *{box-sizing:border-box;}html,body{height:100%;}
  body{margin:0;font-family:var(--sans);color:var(--ink);
    background:radial-gradient(120% 90% at 50% 110%, rgba(45,212,191,.14), transparent 60%),
    linear-gradient(180deg, var(--bg0) 0%, var(--bg1) 55%, #0a1128 100%);overflow:hidden;-webkit-font-smoothing:antialiased;}
  ...
  .badge{display:inline-flex;align-items:center;gap:.6rem;font-family:'Product Sans Medium',var(--sans);font-weight:500;font-size:.8rem;letter-spacing:.42em;text-transform:uppercase;
    background:linear-gradient(90deg,#9ff5e6,var(--sky),#c4b5fd);-webkit-background-clip:text;background-clip:text;color:transparent;animation:rise .9s .1s cubic-bezier(.2,.7,.3,1) both;}
...
<script>
  ...
  const extBtn = document.getElementById('externalBtn');const soon   = document.getElementById('comingSoon');
