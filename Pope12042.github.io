...
<!-- Google Sans ships as 'Product Sans' on cdnfonts -->
...
<style>
  :root{
    ...
    --sans:'Product Sans','Google Sans',var(--fallback);
  }
  ...
  *{box-sizing:border-box;}
  ...
  body{
    ...
    color:var(--ink);
    background:
      radial-gradient(120% 90% at 50% 110%, rgba(45,212,191,.14), transparent 60%),
      linear-gradient(180deg, var(--bg0) 0%, var(--bg1) 55%, #0a1128 100%);
    overflow:hidden;
  ...
  .vignette{position:fixed;inset:0;z-index:3;pointer-events:none;
    background:radial-gradient(120% 120% at 50% 40%, transparent 55%, rgba(2,4,12,.55) 100%);}
  .noise{position:fixed;inset:0;z-index:30;pointer-events:none;opacity:.05;mix-blend-mode:overlay;
  ...
  /* ---------- content ---------- */
  .scene{position:relative;z-index:10;min-height:100svh;display:flex;flex-direction:column;
    align-items:center;justify-content:center;text-align:center;padding:2rem 1.5rem;gap:1.1rem;}

  .badge{display:inline-flex;align-items:center;gap:.6rem;font-family:'Product Sans Medium',var(--sans);
    font-weight:500;font-size:.8rem;letter-spacing:.42em;text-transform:uppercase;
    background:linear-gradient(90deg,#9ff5e6,var(--sky),#c4b5fd);
    -webkit-background-clip:text;background-clip:text;color:transparent;
    animation:rise .9s .1s cubic-bezier(.2,.7,.3,1) both;}
  .badge::before,.badge::after{content:"";height:1px;width:34px;background:linear-gradient(90deg,transparent,rgba(160,200,255,.5));}
  .badge::after{background:linear-gradient(90deg,rgba(160,200,255,.5),transparent);}

  .headline{margin:0;font-family:'Product Sans Black','Google Sans',var(--sans);font-weight:900;
    font-size:clamp(2.1rem,6vw,4.4rem);line-height:1.06;letter-spacing:-0.01em;max-width:16ch;
    color:#fff;text-shadow:0 4px 40px rgba(120,180,255,.25);
    animation:rise .9s .22s cubic-bezier(.2,.7,.3,1) both;}

  .sub{margin:0;font-family:var(--sans);font-weight:400;font-size:clamp(1.25rem,3.4vw,2.1rem);
    background:linear-gradient(180deg,#9fd4ff,var(--blue) 60%,#4a9eff);
    -webkit-background-clip:text;background-clip:text;color:transparent;
    filter:drop-shadow(0 2px 18px rgba(90,170,255,.35));
    animation:rise .9s .34s cubic-bezier(.2,.7,.3,1) both;}

  .actions{display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:.9rem;margin-top:1.4rem;
    animation:rise .9s .46s cubic-bezier(.2,.7,.3,1) both;}

  .btn{display:inline-flex;align-items:center;gap:.6rem;padding:.95rem 1.7rem;border-radius:14px;
    font-family:'Product Sans Medium','Google Sans',var(--sans);font-weight:500;font-size:1.02rem;
    letter-spacing:.01em;text-decoration:none;cursor:pointer;border:1px solid transparent;
    transition:transform .28s cubic-bezier(.2,.7,.3,1), box-shadow .28s ease, background .28s ease, border-color .28s ease;}
  .btn svg{width:1.15rem;height:1.15rem;flex:none;transition:transform .28s ease;}
  .btn:active{transform:translateY(0) scale(.98);}
  .btn:focus-visible{outline:2px solid var(--blue);outline-offset:3px;}

  .btn-primary{color:#04121a;background:linear-gradient(135deg,#5eead4,var(--sky) 55%,#818cf8);
    box-shadow:0 10px 34px rgba(56,189,248,.4), inset 0 1px 0 rgba(255,255,255,.5);}
  ...
  .btn-ghost{color:#e6f1ff;background:rgba(148,197,255,.07);border-color:rgba(148,197,255,.35);
    backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);}
  .btn-ghost:hover{transform:translateY(-3px);background:rgba(148,197,255,.14);
    border-color:rgba(170,210,255,.6);box-shadow:0 12px 32px rgba(90,160,255,.22);}
  .btn-ghost:hover svg{transform:translate(1px,-1px);}

  .coming-soon{margin-top:.4rem;min-height:1.6rem;font-family:'Product Sans Medium',var(--sans);
    font-weight:500;font-size:1rem;letter-spacing:.28em;text-transform:uppercase;
    background:linear-gradient(90deg,#9ff5e6,var(--sky),#c4b5fd,#9ff5e6);background-size:220% auto;
    -webkit-background-clip:text;background-clip:text;color:transparent;
  ...
  @media (prefers-reduced-motion: reduce){
    .ribbon,.badge,.headline,.sub,.actions,.coming-soon.show{animation:none !important;}
    .btn,.btn svg{transition:none !important;}
    .coming-soon{transition:opacity .2s linear;transform:none;}
  }
</style>
</head>
<body>
  ...
  <div class="mountains" id="mountains">
    <svg viewBox="0 0 1440 320" preserveAspectRatio="xMidYMax slice" aria-hidden="true">
      ...
      <g fill="#04060f">
        <path d="M96 320 L116 246 L136 320 Z"/>
        <path d="M150 320 L166 262 L182 320 Z"/>
        <path d="M1180 320 L1200 250 L1220 320 Z"/>
        <path d="M1240 320 L1256 268 L1272 320 Z"/>
        <path d="M620 320 L638 258 L656 320 Z"/>
      </g>
    </svg>
  </div>
...
<script>
  ...
  const mountEl  = document.getElementById('mountains');
  ...
  function tick(){
    ...
    for(const f of flakes){
      f.y += f.s;
      f.x += Math.sin(t*f.w*2 + f.p)*0.3 + f.d;
    ...
    auroraEl.style.transform = 'translate3d(' + (mx*-22) + 'px,' + (my*-14) + 'px,0)';
    mountEl.style.transform  = 'translate3d(' + (mx*16) + 'px,' + (my*9) + 'px,0)';
    requestAnimationFrame(tick);
  }

  if(REDUCED){ draw(); } else { tick(); }

  /* ---- "Get external" → coming soon ---- */
  const extBtn = document.getElementById('externalBtn');
  const soon   = document.getElementById('comingSoon');
  extBtn.addEventListener('click', ()=> soon.classList.toggle('show'));
</script>
</body>
</html>
